...
export default function BurnChart({ posts, itemsByClient, weeklyData, blogPerWeek }) {
  const W = 640, H = 180, padL = 36, padB = 34, padT = 12;
  const chartW = W - padL - 16, chartH = H - padT - padB;
  const maxTarget = Math.max(...[1, 2, 3, 4].map((w) => weekTarget(w, weeklyData, blogPerWeek))) * 1.15;
    ...
    return (
      <g key={w}>
        ...
        <text x={x + barW / 2} y={padT + chartH + 28} fontSize="8.5" fill="#C93636" textAnchor="middle" fontFamily="Inter, sans-serif">
          ~{fmtHours(weekMinutes(w, weeklyData, blogPerWeek))}
        </text>
      </g>
    );
  });
